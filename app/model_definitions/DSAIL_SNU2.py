...
# Has been adapted to fix static export failures, and only include default values.
...
class DSAIL_SNU2(nn.Module):
    """ ECG_model """
    def __init__(self):
        super(DSAIL_SNU2, self).__init__()

        self.standardizer = 'minMax'
        ...
        self.conv1 = conv_kx1(12, nGroups[0], cfg["kernel_size"], stride=1)
        ...
        self.features = torch.tensor([[0.603661872, 0.47138388, 0.52861612, 1, 1]], dtype=torch.float32)

    def to(self, device):
        '''Override the original .to() method to also move self.features to the specified device'''
        self.features = self.features.to(device)
        self.device = device
        return super(DSAIL_SNU2, self).to(device)
    ...
    def forward(self, x, flags=None):
        ...
        # set default age, sex according to paper
        
        features = self.features.repeat((x.shape[0], 1))
